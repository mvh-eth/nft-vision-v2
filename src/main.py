...
def refresh():
    ...
    ''' top_collections = top_collections[top_collections['slug'] == "mutant-ape-yacht-club"]
    
    top_collections = top_collections.to_dict('records')  '''
...
async def get_collection_listings(slug: str):
    return bq.read_mongo(f"{slug}_listings", query_sort=[("createdAt", -1)], query_limit=100)
